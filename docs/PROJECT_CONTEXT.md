...
| Per-report access shapes the UI; `dump` is admin-only | Hrishi: admin sees all reports, cashier gets one default (inhand), everyone else only what admin grants per report. Reports are read-only (no write path); everyone keeps their own-device totals. **Corrected 2026-07-29 (audit §7):** this row used to say report data "must never leak through a hidden button, so the server gates it", which contradicted `residual-risks.md:77` twelve rows away and was simply not true — `pull` requires an approved user and nothing more, and returns the whole year. That is not a hole to close, it is the same decision as **"Grants decide what you may ENTER, not what you may SEE"** (Hrishi, commit `7a84c76`, recorded further down this file): this is the committee's own book and they are on the committee. Grants decide what you may ENTER. The only real server gate is `dump`, which is admin-only. |
| **The Google account is Hrishi's; handover is deferred** (2026-07-31) | Audit #4 Q5 asked who owns the data if the committee changes hands. Hrishi's answer: *"the Google account is mine, it will be handled later as configuration."* Recorded as decided, not open. What it means today: the Sheet, the Drive backups and the Apps Script deployment all live in one personal account, and every phone reaches them through `SCRIPT_URL` in `js/config.js` — so a future handover is a re-deploy plus a config rebake, which is the same operation already performed on every server change. Nothing in the code assumes a particular account. The part that is NOT configuration is the data already in that account, which is why this is worth one written sentence between the committee while everybody is friendly — Hrishi's call on when |
