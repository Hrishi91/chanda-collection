...
(function () {
  ...
  function banglaNumWords(num) {
    ...
    // Hrishi's call (A215). Indexed 1–9 so a missing entry is an obvious
    // `undefined`, never a silently-wrong word.
    const HUNDREDS = ['', 'একশো', 'দুশো', 'তিনশো', 'চারশো', 'পাঁচশো', 'ছশো', 'সাতশো', 'আটশো', 'নশো'];
    if (num === 0) return O[0];
    const p = [];
    ...
    // A215: the hundreds are their own words, not "<digit> শো". Written out
    // Bengali says একশো / দুশো, and a receipt is the one place in this app
    // where the wording is read by somebody outside the committee.
    if (hu) p.push(HUNDREDS[hu]);
    if (num) p.push(O[num]);
    return p.join(' ');
  }
